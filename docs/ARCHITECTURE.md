...
- **`feature/<name>`** — one branch per feature or `PLAN.md` phase, always named `feature/<the-update-or-feature-or-phase>` (e.g. `feature/attendance-engine`, `feature/phase-2-students-staff`), branched from `develop`. Per `RULES.md` #13, this branch is *reused* for all follow-up commits and review fixes on that same slice of work — a new branch is cut only when starting genuinely new, separable work, not for every small change.
